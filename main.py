...
def load_data():
    try:
        ...
        df['課程名稱'] = df['科目簡稱'].astype(str) + " (" + df['開課班級'].astype(str) + ")"
...
if "workload_choice" not in st.session_state: st.session_state.workload_choice = "適中 😊"
...
if st.session_state.current_page == "系統首頁":
    ...
    with st.container(border=True):
        if not has_valid_filter:
            st.info("👈 請先從上方依序完成選擇，系統才會載入分析資料。")
    ...
    with st.container(border=True):
        ...
        with col_right:
            values_closed = radar_data.get(course_info['選課代號'] if target_course_name else None, [0]*5)
    ...
    target_name = st.session_state.get('saved_course')
    
    matches = data[data['選課代號'].astype(str) == str(target_id)]
    if matches.empty and target_name:
        matches = data[data['課程名稱'].str.contains(target_name, na=False)]

    if matches.empty:
        st.warning("⚠️ 找不到該課程資訊，請回視覺化介面重新選擇。")
    else:
        course_data = matches.iloc[0]
        current_code = str(course_data['選課代號'])
        selected_uid = course_data.get('UID', f"[{current_code}] {course_data['課程名稱']}")
        ...
        with st.container(border=True):
            ...
            display_index = 0
            for i, (col_name, val) in enumerate(course_data.items()):
        ...
        with c1: 
            if st.button("❤️ 加入收藏", use_container_width=True):
                if any(c['id'] == current_code for c in st.session_state.my_courses): st.toast(f"已經在清單中囉！", icon="⚠️")
                else:
                    ...
                    st.session_state.my_courses.append({"id": current_code, "name": course_data['課程名稱'], "time": time_slots, "credits": credits, "type": c_type, "enrolled": False})
                    st.toast(f"已加入收藏！", icon="✨")
        with c2: 
    ...
    if len(all_favorites) == 0:
        ...
        # --- 擴大課表安全範圍並加入 .get() 保護 ---
        days = ["一", "二", "三", "四", "五", "六", "日"]
        periods = [str(i) for i in range(1, 16)] + ["A", "B", "C", "D", "E"]
        ...
        for course in enrolled_courses:
            ...
            for t in course["time"]:
                if len(t) >= 2:
                    day, p_raw = t[0], t[1:]
                    p = str(int(p_raw)) if p_raw.isdigit() else p_raw.upper()
                    # 安全寫入矩陣
                    if day in schedule_matrix and p in schedule_matrix[day]: 
                        schedule_matrix[day][p].append(course)
                    ...
                    if is_enrolled:
                        for t in course["time"]:
                            if len(t) >= 2:
                                d, p_raw = t[0], t[1:]
                                p = str(int(p_raw)) if p_raw.isdigit() else p_raw.upper()
                                if len(schedule_matrix.get(d, {}).get(p, [])) > 1:
                                    is_conf = True
                                    break
    ...
    def reset_all_prefs():
        for key in list(st.session_state.keys()):
            if key.startswith("prof_") or key.startswith("cross_") or key.startswith("course_"): st.session_state[key] = False
    ...
    with st.container(border=True):
        ...
        with col1:
            ...
            if st.button("📸 更換頭像", use_container_width=True):
                ...
                if uploaded_file: st.success(f"👍 頭像已上傳：{uploaded_file.name}")
        with col2:
            ...
            with col_grad1: st.write("**必修 (75%)**"); st.progress(0.75)
            with col_grad2: st.write("**選修 (85%)**"); st.progress(0.85)
    ...
    with st.container(border=True):
        ...
        with col_interest_left:
            ...
            prof_fields = {"🏭 生產與製造": False, "📈 品質管理": False, "💻 程式與資訊": False, "📊 數據分析": False, "⚙️ 系統模擬": False, "💼 科技管理": False}
            for field, selected in prof_fields.items(): st.checkbox(field, value=selected, key=f"prof_{field}")
        with col_interest_right:
            st.markdown("##### 跨域/通識偏好 (向度分類)")
            cross_fields = {"🌍 人文與歷史": False, "🎨 藝術與美學": False, "🏛️ 社會與心理": False, "⚖️ 法律與政治": False, "💰 經濟與商管": False, "🌱 自然與環境": False, "🗣️ 外語能力": False}
            for field, selected in cross_fields.items(): st.checkbox(field, value=selected, key=f"cross_{field}")
    ...
    with st.container(border=True):
        ...
        with col_pref1:
            ...
            workload = st.radio("選擇作業負擔程度", workload_options, key="workload_choice", label_visibility="collapsed", horizontal=True)
        ...
        with col_pref2:
            ...
            for i, course in enumerate(course_types):
                with cols_type[i]: st.checkbox(course, value=False, key=f"course_{course}")
