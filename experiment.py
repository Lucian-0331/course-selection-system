...
def get_fixed_trend_data(course_code, difficulty):
    ...
    # 🌟 根據難度，動態調整歷年「平均分數」的合理區間
    if difficulty >= 4.0:
        base_min, base_max = 55, 72  # 硬課：平均在及格邊緣徘徊
    elif difficulty <= 3.0:
        base_min, base_max = 80, 96  # 涼/甜課：平均超高
    else:
        base_min, base_max = 68, 85  # 適中：中規中矩
        
    return pd.DataFrame({
        "Year": ['109', '110', '111', '112', '113'], 
        "Students": [random.randint(40, 120) for _ in range(5)], 
        "AvgScore": [random.randint(base_min, base_max) for _ in range(5)]
    })
...
def get_fixed_grade_dist_data(course_code, difficulty):
    ...
    # 🌟 根據難度，賦予截然不同的「常態分佈權重」
    if difficulty >= 4.0:
        # 💀 硬課：當掉一堆人(0-59高)，分數多集中在 50-70，幾乎沒人 90+
        weights = [1, 2, 5, 8, 15, 25, 25, 12, 5, 2] 
    elif difficulty <= 3.0:
        # 👼 甜課：幾乎沒人不及格，分數瘋狂右偏集中在 80-100
        weights = [0, 0, 0, 1, 2, 3, 10, 25, 40, 19]
    else:
        # 😊 適中：標準常態分佈，集中在 70-89
        weights = [0, 0, 1, 2, 4, 8, 20, 35, 20, 10]
...
def generate_fake_comments(course_code, difficulty, engagement):
    # 確保同一門課每次點開看到的評論都是固定的
    random.seed(course_code)  
    
    # 擴充使用者暱稱庫，確保抽 7 個不會重複
    names = [
        "工工三甲小神童", "期末被當專業戶", "逢甲路過小精靈", "學分小偷", 
        "準時下課推廣大使", "坐在第一排的學霸", "熬夜寫code的肝苦人", 
        "不想延畢的大四老屁股", "只求及格的男子", "圖書館地縛靈", 
        "夜衝第一名", "通識獵人", "靠早八續命", "報告雷隊友剋星", "GPA守護者"
    ]
    ...
    if difficulty >= 4.0:
        diff_pool = [
            "這門課真的硬，期中考都是申論題，沒有看熟上課簡報會死得很慘。",
            "給分很不甜...我全勤加上作業都有用心寫，最後只拿剛好及格。",
            "超級硬課！每次上課都在瘋狂抄筆記，腦容量要夠大。",
            "建議沒有心理準備不要輕易嘗試，期末專案要求非常嚴格。",
            "Loading 超重，每個禮拜都有作業，常常寫到半夜。",
            "老師標準定很高，分數完全不放水，想拿高分要拚老命。",
            "考試範圍超級廣，考前抱佛腳絕對沒救。",
            "雖然很硬，但上完會覺得抗壓性變高了...",
            "被當率不低，選之前真的要評估一下這學期的學分重不重。",
            "涼度是零，每一堂課都在高速運轉，不能恍神。"
        ]
    elif difficulty <= 3.0:
        diff_pool = [
            "給分超甜！老師根本是活菩薩，基本上有交作業都會過。",
            "超級涼課～很適合拿來湊學分，期中也是開書考。",
            "只要出席率正常，考前看個一兩天就能輕鬆拿高分了。",
            "難度不高，老師考前會大洩題，背熟就對了。",
            "佛心老師推推，報告有認真上台講分數都很漂亮。",
            "根本是快樂學分班，沒有太大的壓力。",
            "上課聽得懂，考題都很基本，不會出刁鑽的陷阱題。",
            "缺學分的選這堂就對了，躺著過。",
            "loading 很輕，一學期才兩次小作業，負擔超小。",
            "老師給分很大方，期末成績出來比預期的還高很多。"
        ]
    else:
        diff_pool = [
            "難度適中，有花時間讀書的話分數不會太難看。",
            "中規中矩的課，考試都從投影片出，給分合理。",
            "不是涼課但也沒到硬，作業負擔一般般。",
            "分數給得算公允，一分耕耘一分收穫。",
            "只要上課有聽，考前稍微複習一下就能穩穩拿學分。",
            "不會刻意刁難學生，跟著進度走就沒問題。",
            "難易度剛剛好，不會讓人覺得太廢，也不會壓力太大。",
            "算是偏扎實的課，但只要肯花時間一定會過。",
            "普普通通，沒有特別甜也沒有特別硬。"
        ]
    ...
    if engagement >= 4.0:
        eng_pool = [
            "上課節奏很快，互動超多，常常要小組討論，完全不會想睡覺！",
            "老師很愛點人回答問題，強迫你一定要專心上課。",
            "每堂課都有隨堂小活動或 Kahoot，參與感拉滿。",
            "這門課極度吃重團隊合作，遇到好隊友帶你上天堂。",
            "課堂氣氛很熱絡，老師很會帶動討論。",
            "發言都有加分，大家搶答超踴躍，絕對睡不著。",
            "幾乎每週都要上台短短分享一下，訓練膽量的好地方。",
            "老師超幽默，會一直丟問題下來，整堂課絕無冷場。",
            "滿滿的實作跟討論，很喜歡這種不會死氣沉沉的上課方式。"
        ]
    elif engagement <= 3.0:
        eng_pool = [
            "大班演講課感很重，老師基本上一直唸投影片。",
            "可以在下面做自己的事，老師不太管下面在幹嘛。",
            "上課有點催眠，不需要太多互動，適合喜歡安靜聽課的人。",
            "就是一直聽課做筆記，沒什麼分組或發言的機會。",
            "老師自己講自己的，底下睡成一片也不會管。",
            "滿傳統的授課方式，適合習慣填鴨式教學的人。",
            "節奏偏慢，很容易恍神滑手機。",
            "幾乎零互動，期中跟期末有去考就好。",
            "點完名就可以做自己的事了，非常自由。"
        ]
        ...
        eng_pool = [
            "老師偶爾會丟問題給大家想，互動頻率剛剛好。",
            "上課氛圍還算輕鬆，不會有一直被盯著的壓力。",
            "會有簡單的分組討論，但不會佔用太多時間。",
            "老師會看大家反應調整講課速度，還算能跟上。",
            "偶爾會叫大家舉手回答，但不強迫，整體算隨和。",
            "互動不多不少，剛剛好適合不喜歡太嗨但又不想睡著的人。",
            "有時候會放影片給大家看再寫心得，滿輕鬆的。",
            "算是很典型的點名、聽課、下課的節奏。"
        ]

    # --- 3. 學習收穫評論池 (萬用型，抽 2 則) ---
    harvest_pool = [
        "雖然期末有點趕，但整個做完回頭看，真的覺得自己成長很多。",
        "老師教的東西很貼近實務，感覺以後出社會用得到。",
        "這門課讓我對這個領域完全改觀，重新找回學習的熱忱。",
        "以前覺得很抽象的理論，上完這堂課竟然瞬間通了。",
        "學到了很多解決問題的思維方式，不是只背死書而已。",
        "最大的收穫是學會怎麼把不同科目的知識串起來用。",
        "老師分享了很多他以前在業界的經驗，這些都是書本上學不到的。",
        "過程中踩了很多坑，但解決之後成就感超大，非常值得！",
        "推翻了我很多以前的刻板印象，整個視野都被打開了。",
        "上課的每一週都覺得有接收到新的東西，非常充實的一學期。",
        "訓練了很棒的邏輯分析能力，對未來做專題或寫報告超級有幫助。",
        "就算不是本科系的去聽，也能帶走一套很有用的思考框架。"
    ]

    # --- 4. 通用日常評論池 (抽 1 則) ---
    general_pool = [
        "老師人很好，下課問問題都很有耐心解答。",
        "助教批改作業的速度很快，而且給的 feedback 很詳細。",
        "教室冷氣超冷，記得帶外套...",
        "早八的課真的起不來，但我還是為了這堂課硬撐過去了。",
        "每次搶這堂課都搶破頭，能選到真的運氣好。",
        "課本滿厚的，建議去買二手書比較划算。",
        "教室插座不多，帶筆電去的話記得先充飽電。",
        "老師偶爾會遲到個幾分鐘，但通常都會準時下課。",
        "期中考前一週會有重點大放送，千萬不能翹課！",
        "點名方式是用傳簽到單的，有時候會忘記簽。",
        "上完這堂課剛好可以去排學餐，時間完美。",
        "老師講話聲音滿輕柔的，坐太後面會聽不太清楚。"
    ]
    ...
    # 按照黃金比例精準抽取，確保變數穩定出現
    final_comments.extend(random.sample(harvest_pool, 2))  # 學習收穫 2 則
    final_comments.extend(random.sample(diff_pool, 2))     # 難度 2 則
    final_comments.extend(random.sample(eng_pool, 2))      # 參與性 2 則
    final_comments.extend(random.sample(general_pool, 1))  # 通用 1 則
